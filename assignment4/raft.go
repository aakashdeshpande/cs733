...
func NewCommit(Index int64, Data []byte, err error, Leader bool) Commit{
	var s Commit
	s.Index = Index
	s.Data = Data
	s.err = err
...
func (sm *StateMachine) Commit(Index int64, Leader bool) {
	if(Index != -1) {
		sm.actionCh <- NewCommit(Index, sm.log[Index], nil, Leader)
	} else {
		sm.actionCh <- NewCommit(Index, nil, nil, Leader)
	}
}
...
func (sm *StateMachine) LogStore(Index int64, Entries []byte, EntryTerm int64) {
	...
	//fmt.Println("Log ", sm.id, Index, string(Entries))
...
func (msg AppendEntriesRequest) execute(sm *StateMachine) {
	...
	if  sm.Term > msg.Term {
		sm.Send(msg.LeaderID, NewAppendEntriesResp(sm.id, sm.Term, msg.LastLogIndex, false))
	} else if (msg.LastLogIndex < sm.commitIndex){
		sm.Send(msg.LeaderID, NewAppendEntriesResp(sm.id, sm.Term, sm.commitIndex, true))
	} else if (msg.LastLogIndex != -1) && 
	 (msg.Entries != nil && (msg.LastLogIndex > sm.LastLogIndex || sm.logTerm[msg.LastLogIndex] != msg.LastLogTerm)) {
		//fmt.Println(msg.LastLogIndex, sm.commitIndex, sm.LastLogIndex, sm.logTerm[msg.LastLogIndex], msg.LastLogTerm)
		sm.Send(msg.LeaderID, NewAppendEntriesResp(sm.id, sm.Term, msg.LastLogIndex, false))
	} else {
		if(msg.Entries != nil){
		...
		if sm.commitIndex < msg.LeaderCommit {	// with Success or outside?
			...
			sm.Commit(sm.commitIndex, false)
			 //, Data, err)
		...
		if msg.Success {
	        ...
	        if count > sm.servers/2 {
	        	if sm.commitIndex < msg.Index && sm.Term == sm.logTerm[msg.Index] {
	        		...
	        		sm.Commit(sm.commitIndex, true) 
	        	}
	        }
	    } else { //If AppendEntries fails because of log inconsistency
	    	if(msg.Index < sm.nextIndex[msg.From]) {
	    		sm.nextIndex[msg.From] = msg.Index
	    	}
...
func serverMain(actionCh chan events){
	...
	defer currentTerm.Close()
	// Database to store votedFor
	votedFor, _ := leveldb.OpenFile("votedFor", nil)
	defer votedFor.Close()

	lg, _ := log.Open("Log" + strconv.Itoa(0))
