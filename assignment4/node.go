...
func (rn *RaftNode) process(ev events) {
	if (ev.eventName() == "Alarm") {
		...
		ev, _ := ev.(Commit)
		out := CommitInfo{ev.Data, ev.Index, ev.err, ev.Leader}
		//fmt.Println("Commit ", ev.Index)
		rn.commitCh <- out
...
func (rn *RaftNode) processEvents() {
	//fmt.Println("Raft Node started ", rn.sm.id)
...
func (rn *RaftNode) Append(data []byte) {
    ...
    if (n.Kind == 0) {
		...
		rn.commitCh <- out
    } else {rn.clientCh <- NewAppend(data)}
}
...
// Generates a mock cluster of 3 raft nodes 
func makeMockRafts() ([]RaftNode, *mock.MockCluster){
	...
	for id := 0; id < 3; id++ {
		config := NodeConfig{clconfig, id, "Logs/", 500}
		raftNode := New(config) // 
